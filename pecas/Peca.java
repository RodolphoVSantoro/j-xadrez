...
public abstract class Peca implements Cloneable {
    ...
    public void recupera(boolean ehIA) {
        this.capturada = false;
        if (!ehIA)
            this.sprite.move(this.posicaoTabuleiro.x * Config.LARGURA_PECA,
                    this.posicaoTabuleiro.y * Config.ALTURA_PECA);
    }
    ...
    /**
     * Método para verificar a validade dos movimentos possíveis de uma peça. O
     * método analisa todas
     * as peças de ambas as cores, verificando se um movimento potencial resultaria
     * em uma situação
     * onde o rei da mesma cor que a peça está em cheque. Se isso ocorrer, o
     * movimento é removido
     * da lista de movimentos possíveis.
     *
     * @param movimentosPossiveis Uma lista de posições possíveis que a peça pode
     *                            ocupar.
     * @return Uma lista atualizada de movimentos possíveis, excluindo aqueles que
     *         colocariam o rei
    ...
    protected ArrayList<Posicao> checaValidadeMovimento(ArrayList<Posicao> movimentosPossiveis) {
        ...
        int xRei = tabu.getPecas(this.getCor()).stream().filter(p -> p.getTipoPeca() == TipoPeca.REI).findFirst().get()
                .getPosicaoTabuleiro().x;
        int yRei = tabu.getPecas(this.getCor()).stream().filter(p -> p.getTipoPeca() == TipoPeca.REI).findFirst().get()
                .getPosicaoTabuleiro().y;
        ...
        for (Posicao movimento : movimentosPossiveis) {
            ...
            if (pecaCapturada == null && !movimento.duplo) {
                ...
                if (tabu.getPecas(this.getCor() == Cor.BRANCO ? Cor.PRETO : Cor.BRANCO).stream()
                        .anyMatch(p -> p.tipoPromocao != TipoPeca.PEAO && p.capturada == false
                                && p.getMovimentosPossiveis(true).stream().anyMatch(m -> m.x == xRei && m.y == yRei)))
                    tiraMovimento.add(movimento);
                else if (tabu.getPecas(this.getCor() == Cor.BRANCO ? Cor.PRETO : Cor.BRANCO).stream()
                        .anyMatch(p -> p.tipoPromocao == TipoPeca.PEAO && p.capturada == false
                                && (p.getPosicaoTabuleiro().x + 1 == xRei || p.getPosicaoTabuleiro().x - 1 == xRei)
                                && p.getPosicaoTabuleiro().y + p.dir == yRei))
                    tiraMovimento.add(movimento);
                tabu.posicoesPecas[movimento.x][movimento.y] = null;
            } else if (!movimento.duplo) {
                tabu.posicoesPecas[movimento.x][movimento.y] = this;
                if (tabu.getPecas(this.getCor() == Cor.BRANCO ? Cor.PRETO : Cor.BRANCO).stream()
                        .anyMatch(p -> p != pecaCapturada && p.tipoPromocao != TipoPeca.PEAO && p.capturada == false
                                && p.getMovimentosPossiveis(true).stream().anyMatch(m -> m.x == xRei && m.y == yRei)))
                    tiraMovimento.add(movimento);
                else if (tabu.getPecas(this.getCor() == Cor.BRANCO ? Cor.PRETO : Cor.BRANCO).stream()
                        .anyMatch(p -> p != pecaCapturada && p.tipoPromocao == TipoPeca.PEAO && p.capturada == false
            ...
            if (movimento.duplo) {
                ...
                if (pecaCapturada2 == null) {
                    ...
                    if (tabu.getPecas(this.getCor() == Cor.BRANCO ? Cor.PRETO : Cor.BRANCO).stream()
                            .anyMatch(p -> p.tipoPromocao != TipoPeca.PEAO && p.capturada == false && p
                                    .getMovimentosPossiveis(true).stream().anyMatch(m -> m.x == xRei && m.y == yRei)))
                        tiraMovimento.add(movimento);
                    else if (tabu.getPecas(this.getCor() == Cor.BRANCO ? Cor.PRETO : Cor.BRANCO).stream()
                            .anyMatch(p -> p.tipoPromocao == TipoPeca.PEAO && p.capturada == false
                                    && (p.getPosicaoTabuleiro().x + 1 == xRei || p.getPosicaoTabuleiro().x - 1 == xRei)
                    ...
                    tabu.posicoesPecas[movimento.x2][movimento.y2] = this;
                    if (tabu.getPecas(this.getCor() == Cor.BRANCO ? Cor.PRETO : Cor.BRANCO).stream().anyMatch(
                            p -> p != pecaCapturada2 && p.tipoPromocao != TipoPeca.PEAO && p.capturada == false && p
                                    .getMovimentosPossiveis(true).stream().anyMatch(m -> m.x == xRei && m.y == yRei)))
                        tiraMovimento.add(movimento);
                    else if (tabu.getPecas(this.getCor() == Cor.BRANCO ? Cor.PRETO : Cor.BRANCO).stream()
                            .anyMatch(p -> p != pecaCapturada2 && p.tipoPromocao == TipoPeca.PEAO
                                    && p.capturada == false
                                    && (p.getPosicaoTabuleiro().x + 1 == xRei || p.getPosicaoTabuleiro().x - 1 == xRei)
                                    && p.getPosicaoTabuleiro().y + p.dir == yRei))
                        tiraMovimento.add(movimento);
