...
public class MaquinaDeRegras {
    ...
    /**
     * Este método verifica se o fim de jogo foi alcançado no tabuleiro de xadrez
     * atual.
     * A condição de término do jogo é quando um rei está em checkmate, ou seja,
     * está sob ataque e não pode fazer um movimento válido sem continuar em cheque.
     *
     * O método percorre todas as peças brancas e pretas do tabuleiro e verifica se
     * o rei de cada cor está em cheque e não pode fazer um movimento válido.
     * 
     * Se um rei está em cheque e não pode fazer um movimento válido, então o jogo
     * terminou.
     * 
     * @return Um array booleano com dois elementos. O primeiro elemento é
     *         verdadeiro se as peças brancas estão em cheque mate. O segundo
     *         elemento é verdadeiro se as peças pretas estão em cheque mate.
     * @throws CloneNotSupportedException Se um erro ocorrer durante a clonagem de
     *                                    uma peça.
     */
    public boolean[] chegouFimDeJogo() {
        ...
        for (Peca peca : this.tabuleiro.getPecas(Cor.BRANCO)) {
            try {
                if (peca.getTipoPeca() == TipoPeca.PEAO)
                    pecaBranca.add((Peao) peca.clone());
                else if (peca.getTipoPeca() == TipoPeca.CAVALO)
                    pecaBranca.add((Cavalo) peca.clone());
                else if (peca.getTipoPeca() == TipoPeca.BISPO)
                    pecaBranca.add((Bispo) peca.clone());
                else if (peca.getTipoPeca() == TipoPeca.TORRE)
                    pecaBranca.add((Torre) peca.clone());
                else if (peca.getTipoPeca() == TipoPeca.DAMA)
                    pecaBranca.add((Dama) peca.clone());
                else if (peca.getTipoPeca() == TipoPeca.REI)
                    pecaBranca.add((Rei) peca.clone());
        ...
        for (Peca peca : this.tabuleiro.getPecas(Cor.PRETO)) {
            try {
                if (peca.getTipoPeca() == TipoPeca.PEAO)
                    pecaPreto.add((Peao) peca.clone());
                else if (peca.getTipoPeca() == TipoPeca.CAVALO)
                    pecaPreto.add((Cavalo) peca.clone());
                else if (peca.getTipoPeca() == TipoPeca.BISPO)
                    pecaPreto.add((Bispo) peca.clone());
                else if (peca.getTipoPeca() == TipoPeca.TORRE)
                    pecaPreto.add((Torre) peca.clone());
                else if (peca.getTipoPeca() == TipoPeca.DAMA)
                    pecaPreto.add((Dama) peca.clone());
                else if (peca.getTipoPeca() == TipoPeca.REI)
                    pecaPreto.add((Rei) peca.clone());
            } catch (CloneNotSupportedException e) {
        ...
        boolean brancoPreso = pecaBranca.stream()
                .allMatch(p -> p.getMovimentosPossiveis(false).isEmpty() || p.capturada);

        boolean brancoCheck = pecaPreto.stream().anyMatch(p -> p.tipoPromocao != TipoPeca.PEAO && !p.capturada
                && p.getMovimentosPossiveis(true).stream().anyMatch(
                        m -> m.x == reiBranco.getPosicaoTabuleiro().x && m.y == reiBranco.getPosicaoTabuleiro().y))
                || this.tabuleiro.getPecas(Cor.PRETO).stream()
                        .anyMatch(p -> p.tipoPromocao == TipoPeca.PEAO && p.capturada == false
                                && (p.getPosicaoTabuleiro().x + 1 == reiBranco.getPosicaoTabuleiro().x
                                        || p.getPosicaoTabuleiro().x - 1 == reiBranco.getPosicaoTabuleiro().x)
                                && p.getPosicaoTabuleiro().y + p.dir == reiBranco.getPosicaoTabuleiro().y);

        boolean pretoPreso = pecaPreto.stream().allMatch(p -> p.getMovimentosPossiveis(false).isEmpty() || p.capturada);
        boolean pretoCheck = pecaBranca.stream().anyMatch(p -> p.tipoPromocao != TipoPeca.PEAO && !p.capturada
                && p.getMovimentosPossiveis(true).stream().anyMatch(
                        m -> m.x == reiPreto.getPosicaoTabuleiro().x && m.y == reiPreto.getPosicaoTabuleiro().y))
                || this.tabuleiro.getPecas(Cor.BRANCO).stream()
                        .anyMatch(p -> p.tipoPromocao == TipoPeca.PEAO && p.capturada == false
                                && (p.getPosicaoTabuleiro().x + 1 == reiPreto.getPosicaoTabuleiro().x
                                        || p.getPosicaoTabuleiro().x - 1 == reiPreto.getPosicaoTabuleiro().x)
                                && p.getPosicaoTabuleiro().y + p.dir == reiPreto.getPosicaoTabuleiro().y);
     ...
     * 4. Todas as peças de uma cor estão bloqueadas (não têm movimentos válidos) e
     * seu rei não está em cheque.
     *
    ...
    public boolean empatouJogo() {
        ...
        boolean presoBranco = pecaBranca.stream()
                .allMatch(p -> p.getMovimentosPossiveis(false).isEmpty() || p.capturada);

        boolean checkBranco = pecaPreto.stream().anyMatch(p -> p.tipoPromocao != TipoPeca.PEAO && !p.capturada
                && p.getMovimentosPossiveis(true).stream().anyMatch(
                        m -> m.x == reiBranco.getPosicaoTabuleiro().x && m.y == reiBranco.getPosicaoTabuleiro().y))
                || this.tabuleiro.getPecas(Cor.PRETO).stream()
                        .anyMatch(p -> p.tipoPromocao == TipoPeca.PEAO && p.capturada == false
        ...
        boolean bispoBranco = pecaBranca.stream().anyMatch(p -> p.tipoPromocao == TipoPeca.BISPO && !p.capturada
                && pecaBranca.stream().allMatch(p2 -> p2 == p || p2.tipoPeca == TipoPeca.REI || p2.capturada));

        boolean cavaloBranco = pecaBranca.stream()
                .anyMatch(p -> p.tipoPromocao == TipoPeca.CAVALO && !p.capturada
                        && pecaBranca.stream().allMatch(p2 -> p2 == p || p2.tipoPeca == TipoPeca.REI
                                || (p2.tipoPromocao == TipoPeca.CAVALO && !p2.capturada) || p2.capturada));

        boolean presoPreto = pecaPreto.stream().allMatch(p -> p.getMovimentosPossiveis(false).isEmpty() || p.capturada);

        boolean checkPreto = pecaBranca.stream().anyMatch(p -> p.tipoPromocao != TipoPeca.PEAO && !p.capturada
                && p.getMovimentosPossiveis(true).stream().anyMatch(
                        m -> m.x == reiPreto.getPosicaoTabuleiro().x && m.y == reiPreto.getPosicaoTabuleiro().y))
                || this.tabuleiro.getPecas(Cor.BRANCO).stream()
                        .anyMatch(p -> p.tipoPromocao == TipoPeca.PEAO && p.capturada == false
        ...
        boolean bispoPreto = pecaPreto.stream().anyMatch(p -> p.tipoPromocao == TipoPeca.BISPO && !p.capturada
                && pecaPreto.stream().allMatch(p2 -> p2 == p || p2.tipoPeca == TipoPeca.REI || p2.capturada));

        boolean cavaloPreto = pecaPreto.stream()
                .anyMatch(p -> p.tipoPromocao == TipoPeca.CAVALO && !p.capturada
                        && pecaPreto.stream().allMatch(p2 -> p2 == p || p2.tipoPeca == TipoPeca.REI
                                || (p2.tipoPromocao == TipoPeca.CAVALO && !p2.capturada) || p2.capturada));

        return (((presoBranco && !checkBranco) || (presoPreto && !checkPreto))
                || ((reiSoloBranco || bispoBranco || cavaloBranco) && (reiSoloPreto || bispoPreto || cavaloPreto)));

    }

    /**
     * Este método executa um movimento no tabuleiro de xadrez.
     * O movimento é considerado válido e será executado se a posição final estiver
     * dentro das posições possíveis para a peça.
     * Se um movimento válido for realizado, o movimento e a peça potencialmente
     * capturada serão adicionados ao histórico de movimentos.
     *
     * @param movimento O movimento a ser realizado.
     * @param ehIA      Um booleano que indica se o movimento é realizado pela
     *                  Inteligência Artificial.
     * @return Verdadeiro se o movimento for válido e for executado com sucesso,
        ...
        if (posicaoValida) {
            ...
            if (movimento.movimentoDuplo) {
                ...
                if (pecaCapturada == null)
                    movimento.getPeca2().qtdMovimento += 1;
            }
     ...
     * Se o último movimento foi um movimento duplo (roque), ambos os movimentos são
     * desfeitos.
     * Caso uma peça tenha sido capturada no último movimento, ela é retornada ao
     * tabuleiro.
     * Se não houver movimentos no histórico para desfazer, um erro será lançado.
     *
     * @param ehIA Um booleano que indica se o movimento a ser desfeito foi
     *             realizado pela Inteligência Artificial.
    ...
    public void desfazUltimoMovimento(boolean ehIA) {
        ...
        if (ultimoMovimento.movimentoDuplo) {
            if (ultimoMovimento.getPecaCapturada() == null)
                ultimoMovimento.getPeca2().qtdMovimento -= 1;
    ...
    /**
     * Este método controla o movimento da Inteligência Artificial (IA) no jogo de
     * xadrez.
     * O método busca o movimento que a IA deseja fazer, executa o movimento e em
     * seguida muda o turno para o jogador humano.
    ...
    public void moveIA() {
        ...
        if (movimento.getPeca() == null)
            System.out.println("IA derrotada");
        else {
