...
public class IA {
    ...
    /**
     * Este método é usado para calcular o melhor movimento possível para a
     * Inteligência Artificial (IA),
     * utilizando o algoritmo MinMax com um determinado nível de profundidade
     * definido em configurações.
     * Além disso, verifica se a peça a ser movida está qualificada para uma
     * promoção, promovendo-a se necessário.
     *
    ...
    /**
     * Este método implementa a parte principal do algoritmo MinMax, que é um
     * algoritmo de busca recursiva usado
     * para tomada de decisões. O algoritmo navega pela árvore de jogo,
     * alternando entre níveis "min" e "max" representando os movimentos dos dois
     * jogadores.
     *
     * @param profundidade É o nível de profundidade até o qual o algoritmo MinMax
     *                     deve pesquisar na árvore do jogo.
     * @param alpha        É o melhor valor encontrado até agora no caminho de busca
     *                     para o jogador MAX.
     * @param beta         É o melhor valor encontrado até agora no caminho de busca
     *                     para o jogador MIN.
     * @param vez          Um valor booleano que determina de quem é a vez,
    ...
    /**
     * Este método implementa a função MAX do algoritmo MinMax. Este método é
     * chamado quando é a vez do jogador MAX.
     * O objetivo é maximizar o valor da função de avaliação, ou seja, encontrar o
     * movimento que dá ao jogador MAX a maior pontuação possível.
     *
    ...
    private Movimento max(int profundidade, double alpha, double beta) {
        ...
        if (profundidade <= 0 || temp[0] || temp[1]) {
            if (temp[0])
                return new Movimento(null, null, null, Integer.MAX_VALUE);
            if (temp[1])
                return new Movimento(null, null, null, Integer.MIN_VALUE);
        ...
        for (Peca peca : pecas) {
            if (!peca.getCapturado()) {
                for (Posicao posicao : peca.getMovimentosPossiveis(false)) {
                    if (peca.tipoPromocao == TipoPeca.PEAO && peca.qtdMovimento == 5)
                        peca.promocao = 9;
                    Movimento novoMovimento = null;
                    if (posicao.duplo) {
                        Peca peca2 = this.tabuleiro.getPeca(posicao.xp, posicao.yp);
                        novoMovimento = new Movimento(peca, peca.getPosicaoTabuleiro(),
                                new Posicao(posicao.x, posicao.y), peca2, peca2.getPosicaoTabuleiro(),
                                new Posicao(posicao.x2, posicao.y2), 0);
                    } else {
                    ...
                    if (!movimentou) {
                        throw new RuntimeException("Movimento inválido computando minMax "
                                ...
                                + " " + novoMovimento.getPosicaoPosterior().x + " "
     ...
     * chamado quando é a vez do jogador MIN.
     * O objetivo é minimizar o valor da função de avaliação, ou seja, encontrar o
     * movimento que dá ao jogador MIN a menor pontuação possível.
     *
     * @param profundidade É a profundidade máxima de busca na árvore de jogos.
     * @param alpha        É o melhor valor encontrado até agora no caminho de busca
     *                     para o jogador MAX.
     ...
     * entre o valor das peças da IA e as do adversário.
     * O valor de cada peça é determinado por um conjunto predefinido de pontuações
     * disponíveis em 'Config.pontuacao'.
     * As peças capturadas são excluídas da avaliação.
     *
     * @return Retorna a diferença entre o valor das peças da IA e o valor das peças
     *         do adversário.
     */
    private int getValorTabuleiro() {
        ...
        for (Peca peca : pecas) {
            if (!peca.getCapturado())
                valorPecas += Config.pontuacao.get(peca.tipoPromocao);
        }
